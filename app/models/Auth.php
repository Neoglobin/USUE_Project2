...
class Auth
{
    ...
    public function generate_token($userId)
    {
        try {
            ...
            // Удаление просроченных токенов обновления
            $stmt = $this->pdo->prepare("DELETE FROM refresh_tokens WHERE user_id = :id");
...
function refresh_token($userId)
{
    date_default_timezone_set('Asia/Yekaterinburg');

    require 'C:/xampp/htdocs/db_connect.php';

    $stmt = $pdo->prepare("SELECT * FROM refresh_tokens WHERE user_id = :id");
    $stmt->bindParam(':id', $userId);
    if ($stmt->execute()) {
        // Проверка "жив" ли токен обновления
        $exp = $stmt->fetch(PDO::FETCH_ASSOC);
        if (strtotime($exp['expires_at']) > strtotime(date('Y-m-d H:i:s', time()))) {
            $refresh = new Auth('', '', $pdo);
            $newToken = $refresh->generate_token($userId);

            return verify_token($newToken);
        } else {
            $_SESSION['auth_failed'] = json_encode(['Verification error' => 'Refresh token expired']);
            header('Location: ../models/logout.php');
            die;
        }
    }
